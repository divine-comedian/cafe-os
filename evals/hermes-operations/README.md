...
The `core` suite contains 22 scenarios—exactly eleven `es-MX` and eleven English—and 35 conversational turns. The partial-request suite also covers a one-confirmation provider + lot + purchase batch. Together they cover reads, calculations, traceability, ambiguity, missing required data, all create flows, natural-language roast-entry capture with unit and elapsed-time conversion, progressive roast and checkpoint patching, browser-local timer boundaries, rejection of lifecycle statuses, roast voiding, guarded and successful deletion, purchase-document upload, dynamic discovery, and stored confirmation. The reviewed ten-tool Cafe catalog has a normalized schema snapshot.

The `partial` suite adds nine scenarios and 24 turns for terse messages and voice-note-like input. It distinguishes required from optional fields, verifies that missing required facts produce one focused question and no partial proposal or mutation, carries sequential follow-up answers into a proposal, confirms that unknown optional fields are omitted rather than invented, and covers close-name recovery after a speech-to-text error. It includes a native three-operation proposal batch with one workflow confirmation as well as provider, purchase, green-lot, roast, and update workflows in English and Mexican Spanish.
...
Tool arguments and persisted state can be checked as partial nested objects with exact recursive array matching for structured roast checkpoints. State checks can assert either presence or absence. This catches wrong IDs, fields, control-point ordering, and targets even when the final prose sounds correct.
