...
function Workspace({ supabase, session }: { supabase: SupabaseClient; session: Session }) {
  ...
  async function save(kind: EntryKind, payload: Record<string, unknown>) {
    const endpoint = { provider: "providers", purchase: "purchases", lot: "green-coffee-lots", roast: "roast-batches" }[kind];
    await api.create(endpoint, payload); setEntry(null);
    setNotice(kind === "purchase" || kind === "roast" ? "Borrador guardado. Revísalo antes de confirmarlo." : "Registro guardado.");
...
function Dashboard({ data, setView, setEntry }: { data: OperationsData; setView: (view: View) => void; setEntry: (kind: EntryKind) => void }) {
  ...
  return <><section className="stats-grid"><Stat label="Café verde registrado" value={weight(green)} detail={data.lots.length + " lotes"} tone="green" /><Stat label="Compras confirmadas" value={money(spend)} detail="Monto acumulado" tone="clay" /><Stat label="Merma promedio" value={avgLoss === null ? "—" : avgLoss.toFixed(1) + "%"} detail={lossValues.length + " tostados"} tone="gold" /><Stat label="Por revisar" value={String(drafts)} detail="Borradores pendientes" tone="ink" /></section>
    <section className="dashboard-grid"><div className="panel"><div className="panel-heading"><p className="eyebrow">Atajos</p><h2>Registrar movimiento</h2></div><div className="quick-grid">{([["provider","P","Proveedor","Nombre y región"],["purchase","$","Compra","Fecha y monto"],["lot","L","Lote verde","Peso y costo por kg"],["roast","R","Tostado","Entrada y salida"]] as Array<[EntryKind,string,string,string]>).map((item) => <button className="quick-action" key={item[0]} onClick={() => setEntry(item[0])}><span className="quick-action__icon">{item[1]}</span><span><strong>{item[2]}</strong><small>{item[3]}</small></span><b>→</b></button>)}</div></div>
...
function Records({ view, data, onConfirm }: { view: Exclude<View, "dashboard">; data: OperationsData; onConfirm: (value: { path: string; title: string; detail: string }) => void }) {
  ...
  const items = view === "providers" ? data.providers.filter((x) => matches(x.name + " " + (x.region || ""))) : view === "purchases" ? data.purchases.filter((x) => matches(provider(x.provider_id) + " " + x.status)) : view === "lots" ? data.lots.filter((x) => matches((x.name || "") + " " + (x.origin || ""))) : data.roasts.filter((x) => matches((x.name || "") + " " + (lot(x.green_coffee_lot_id)?.name || "") + " " + x.status));
  return <section className="panel records-panel"><div className="records-toolbar"><label className="search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar registros…" /></label><span>{items.length} registros</span></div>{items.length === 0 ? <Empty /> :
    ...
    view === "purchases" ? <div className="record-list"><Head labels={["Proveedor","Fecha","Monto","Estado",""]} className="purchase-row" />{(items as Purchase[]).map((x) => <div className="record-row purchase-row" key={x.id}><Primary title={provider(x.provider_id)} detail={"Compra #" + short(x.id)} /><span data-label="Fecha">{showDate(x.purchased_at)}</span><strong data-label="Monto">{money(x.total_amount, x.currency)}</strong><span data-label="Estado"><Status value={x.status} /></span><span>{x.status === "draft" && <button className="button button--small" onClick={() => onConfirm({ path: "purchases/" + x.id + "/confirm", title: "Confirmar compra", detail: provider(x.provider_id) + " · " + money(x.total_amount, x.currency) })}>Revisar</button>}</span></div>)}</div> :
...
function EntryModal({ kind, data, onClose, onSave }: { kind: EntryKind; data: OperationsData; onClose: () => void; onSave: (kind: EntryKind, payload: Record<string, unknown>) => Promise<void> }) {
  ...
  function prepare(event: FormEvent<HTMLFormElement>) {
    ...
    else if (kind === "purchase") value = { provider_id: raw.provider_id, purchased_at: raw.purchased_at, ...(raw.total_amount ? { total_amount: raw.total_amount } : {}), currency: "MXN", ...(raw.payment_method ? { payment_method: raw.payment_method } : {}) };
...
function Fields({ kind, data }: { kind: EntryKind; data: OperationsData }) {
  ...
  if (kind === "purchase") return <div className="form-grid"><Field label="Proveedor" full><select name="provider_id" required defaultValue="" autoFocus><option value="" disabled>Selecciona un proveedor</option>{data.providers.map((x) => <option key={x.id} value={x.id}>{x.name}{x.region ? " · " + x.region : ""}</option>)}</select></Field><Field label="Fecha de compra"><input name="purchased_at" type="date" defaultValue={new Date().toISOString().slice(0,10)} required /></Field><Field label="Total pagado" unit="MXN"><input name="total_amount" type="number" min="0" step="0.01" placeholder="0.00" /></Field><Field label="Forma de pago" optional full><select name="payment_method" defaultValue=""><option value="">Sin especificar</option><option value="transferencia">Transferencia</option><option value="efectivo">Efectivo</option><option value="tarjeta">Tarjeta</option></select></Field><p className="form-note field--full">Se guardará como borrador para revisión.</p></div>;
...
function Proposal({ kind, value, data, busy, error, onBack, onSave }: { kind: EntryKind; value: Record<string, unknown>; data: OperationsData; busy: boolean; error: string; onBack: () => void; onSave: () => void }) {
  ...
  else if (kind === "purchase") { rows.push(["Proveedor",data.providers.find((x) => x.id === value.provider_id)?.name || "—"],["Fecha",showDate(String(value.purchased_at))],["Total",value.total_amount ? money(value.total_amount) : "Pendiente"],["Estado",<Status value="draft" />]); }
