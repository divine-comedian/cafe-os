...
function durationFromParts(minutesValue: unknown, secondsValue: unknown) {
  ...
  return Number(minutesText || 0) * 60 + Number(secondsText || 0);
}
...
function EditRoastModal({ roast, data, onClose, onSave }: { roast: RoastBatch; data: OperationsData; onClose: () => void; onSave: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  const durationSeconds = roast.duration_seconds === null ? null : Math.max(0, Math.round(Number(roast.duration_seconds)));
  const durationMinutes = durationSeconds === null ? "" : Math.floor(durationSeconds / 60);
  const durationRemainder = durationSeconds === null ? "" : durationSeconds % 60;
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    const payload = { green_coffee_lot_id: lotId, name: raw.name ? String(raw.name).trim() : null, roast_date: raw.roast_date || null, roasted_at: isoFromLocalDateTime(raw.roasted_at), green_input_kg: raw.green_input_kg || null, roasted_output_kg: raw.roasted_output_kg || null, duration_seconds: durationFromParts(raw.duration_minutes, raw.duration_remainder_seconds), charge_temperature_c: raw.charge_temperature_c || null, balance_point_temperature_c: raw.balance_point_temperature_c || null, setup_notes: raw.setup_notes ? String(raw.setup_notes).trim() : null, sensory_rating: raw.sensory_rating ? Number(raw.sensory_rating) : null, tasting_notes: raw.tasting_notes ? String(raw.tasting_notes).trim() : null, notes: raw.notes ? String(raw.notes).trim() : null };
  ...
  return <Modal onClose={onClose} title="Editar tostado" eyebrow="Guardar actualiza inventario"><form className="entry-form" onSubmit={submit}><div className="form-grid">
    ...
    <Field label="Duración" unit="min" optional><input name="duration_minutes" type="number" min="0" step="1" defaultValue={durationMinutes} /></Field>
    <Field label="Segundos" unit="seg" optional><input name="duration_remainder_seconds" type="number" min="0" max="59" step="1" defaultValue={durationRemainder} /></Field>
...
function EntryModal({ kind, data, onClose, onSave }: { kind: EntryKind; data: OperationsData; onClose: () => void; onSave: (kind: EntryKind, payload: Record<string, unknown>) => Promise<void> }) {
  ...
  function prepare(event: FormEvent<HTMLFormElement>) {
    ...
    else { const inventory = greenCoffeeInventory(String(raw.green_coffee_lot_id),data.purchases,data.roasts); const greenInput = raw.green_input_kg ? Number(raw.green_input_kg) : null; const roastedOutput = raw.roasted_output_kg ? Number(raw.roasted_output_kg) : null; const durationSeconds = durationFromParts(raw.duration_minutes, raw.duration_remainder_seconds); if (greenInput !== null && roastedOutput !== null && roastedOutput > greenInput) { setError("La salida tostada no puede superar la entrada verde."); return; } if (greenInput !== null && greenInput > inventory.availableKg + 1e-9) { setError(`Solo hay ${weight(inventory.availableKg)} disponibles para este lote.`); return; } value = { green_coffee_lot_id: raw.green_coffee_lot_id, ...(raw.name ? { name: String(raw.name).trim() } : {}), ...(raw.roast_date ? { roast_date: raw.roast_date } : {}), ...(raw.roasted_at ? { roasted_at: isoFromLocalDateTime(raw.roasted_at) } : {}), ...(raw.green_input_kg ? { green_input_kg: raw.green_input_kg } : {}), ...(raw.roasted_output_kg ? { roasted_output_kg: raw.roasted_output_kg } : {}), ...(durationSeconds !== null ? { duration_seconds: durationSeconds } : {}), ...(raw.charge_temperature_c ? { charge_temperature_c: raw.charge_temperature_c } : {}), ...(raw.balance_point_temperature_c ? { balance_point_temperature_c: raw.balance_point_temperature_c } : {}), ...(raw.setup_notes ? { setup_notes: String(raw.setup_notes).trim() } : {}), ...(raw.sensory_rating ? { sensory_rating: Number(raw.sensory_rating) } : {}), ...(raw.tasting_notes ? { tasting_notes: String(raw.tasting_notes).trim() } : {}), ...(raw.notes ? { notes: String(raw.notes).trim() } : {}) }; }
    setProposal(value);
  }
