...
function Records({ view, data, onEditPurchase, onEditRoast }: { view: Exclude<View, "dashboard">; data: OperationsData; onEditPurchase: (purchase: Purchase) => void; onEditRoast: (roast: RoastBatch) => void }) {
  ...
  const items = view === "providers" ? data.providers.filter((x) => matches(x.name + " " + (x.region || ""))) : view === "purchases" ? data.purchases.filter((x) => { const purchasedLot = lot(x.green_coffee_lot_id); return matches(provider(x.provider_id) + " " + (purchasedLot?.name || "") + " " + (purchasedLot?.origin || "") + " " + (purchasedLot?.variety || "")); }) : view === "lots" ? data.lots.filter((x) => matches(x.name + " " + (x.origin || "") + " " + (x.variety || ""))) : data.roasts.filter((x) => matches((x.name || "") + " " + (lot(x.green_coffee_lot_id)?.name || "") + " " + x.status));
