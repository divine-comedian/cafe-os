...
- Roast batch: the dedicated New/History/Panel workspace follows the client's original roasting log. Only the lot is required initially; date, timestamp, weights, duration, charge temperature, balance-point temperature, setup notes, manual control points, sensory rating, tasting notes, and operator notes can be added progressively.

The new-roast workspace keeps its unsaved form, timer state, rating, and control points in browser `localStorage`, scoped to the authenticated operator. The display updates locally every half-second and does not poll or write timer ticks to the API. Start, pause, resume, and reset remain disabled until the selected green-coffee lot resolves to a current record. Timer state and checkpoint entry survive a page refresh on the same browser. The confirmation step performs the single database write. Elapsed time is stored as integer seconds; the UI renders `MM:SS`, adding an hours segment only beyond 59:59.

Each control point contains elapsed seconds, optional temperature, optional airflow/`tiro`, optional gas, and an optional note. Adding a control point defaults to the live timer but permits explicit `MM:SS` or `HH:MM:SS` entry. `Punto de equilibrio` is an optional temperature in °C: the observed minimum after charging where the falling roast curve begins to rise.
