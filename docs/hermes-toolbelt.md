...
In the trusted Telegram deployment, the main model sees the complete ten-tool Cafe catalog on every ordinary turn. This removes classifier omissions when a request changes from a lookup into a write. For a provider + lot + purchase workflow, the model prepares three separate stored proposals in one assistant tool-call batch and presents one combined confirmation. One approval resumes the prerequisite provider and lot confirmations together, then the purchase confirmation after exact-name references resolve to their returned IDs. The middleware combines the authoritative receipts into one response and never invokes Hermes' single-use downstream executor twice. The grant ends when the stored batch completes and never covers deletes or voiding a roast; those always require their own explicit approval. General-purpose Hermes tools remain unavailable. In optional `routed` mode, the boundary limits the active catalog to five classifier-selected tools and uses Cafe-only discovery as its fallback. In either mode, a request allows at most one discovery call. Each tool result is capped at 24,000 serialized characters and aggregate results at 48,000 characters per turn; the harness reserves at most 4,096 completion tokens per model hop and enforces an 8,192-token aggregate turn budget.
